...
const getCowAnalyticsBetweenTwoDates = async (req, res, next) => {
    try {
...
const getTodayCowAnalytics = async (req, res, next) => {
    try {
        ...
        const feedConsumtionRecord = await FeedConsumtion.aggregate([
          ...
          const milkRecord = await MilkProduction.aggregate([
            {
              $match: {
                $and: [
                  { date },
          ...
          //MilkProduction
          const morningMPCount=milkRecord.reduce((sum,record)=>sum+record.morning,0)
          const eveningMPCount=milkRecord.reduce((sum,record)=>sum+record.evening,0)

        //feedConsumtionRecord
        const morningFCCount=feedConsumtionRecord.reduce((sum,record)=>sum+record.morning,0)
        const eveningFCCount=feedConsumtionRecord.reduce((sum,record)=>sum+record.evening,0)
       
        
        res.json({
            success: true,
            message: "Successfully get today cow details milk and consumtiopn  ",
            data: {
                milk: {
                    milkRecord,
                    morningMilkCount:morningMPCount,
                    eveningMilkCount:eveningMPCount,
                    total:morningMPCount+eveningMPCount
                },
                feedConsumtion: {
                    feedConsumtionRecord,
                    morningFeedConsumtionCount:morningFCCount,
                    eveningFeedConsumtionCount:eveningFCCount,
                    total:morningFCCount+eveningFCCount
                }
            },
        });     
...
export { getCowAnalyticsBetweenTwoDates,getTodayCowAnalytics }